...
| `scheduled/`  | Directory of recurring **scheduled tasks** (`*.md` with a schedule in front matter). See below. |
...
## Scheduled tasks (`.orchestrator/scheduled/*.md`)

A project can carry **scheduled tasks**: markdown files under
`.orchestrator/scheduled/`, each describing a recurring job. The schedule lives in
the file's YAML-style front matter; the markdown body is the prompt handed to the
agent when the job fires.

The app discovers these on launch and re-scans every few minutes (the interval is
the `scheduleRefreshSecs` global setting, default 300). Parsing and the
next-run math live in [`crates/core/src/scheduled.rs`](../crates/core/src/scheduled.rs);
discovery, persistence, and firing live in the engine. The **Scheduled** view
lists every discovered job with its next run and lets you pause/enable each.

Example — `.orchestrator/scheduled/daily-deps.md`:

```markdown
---
schedule: "0 9 * * *"   # cron (5 or 6 fields), OR use `every: 6h` for an interval
agent: claude            # optional — omit to let the scheduler balance load
model: opus              # optional — defaults to the agent's latest
priority: high           # low | normal | high | urgent, or a number
enabled: true            # optional, default true
title: Daily dependency check
---
Check for outdated dependencies and open follow-up tasks to update them.
```
...
| Field      | Meaning |
|------------|---------|
| `schedule` / `cron` | A cron expression. Standard 5-field (`min hour dom month dow`) is accepted and normalized to the 6-field form the engine uses; 6-field (seconds-first) also works. |
| `every` / `interval` | A simple interval instead of cron: `30s`, `15m`, `6h`, `1d`, `2w`. |
| `agent` | Optional agent (`claude`/`gemini`/`codex`). If omitted, the fired task is auto/balanced. Ignored if the project doesn't allow that agent. |
| `model` | Optional model override for the fired task. |
| `priority` | `low`/`normal`/`high`/`urgent` or a number; default `50`. |
| `enabled` | `false` pauses the job without deleting the file. |
| `title` | Display title; defaults to the file name. |

When a job is due, the engine creates a normal pending task (tagged `scheduled`)
from the body and advances the schedule. Missed slots (e.g. while the app was
closed) collapse to a single firing — you don't get a backlog burst.

## Allowed agents, load balancing, and models

**Allowed agents.** Each project has an allowed-agent set (default: Claude only),
edited in the project settings. The scheduler never dispatches to an agent outside
this set, and task creation rejects a pinned agent that isn't allowed.

**Load balancing.** When a task does not pin an agent (created as *Auto / balanced*,
or generated by the roadmap loop without an explicit agent), and the project allows
more than one agent, the scheduler picks the **least-used available** agent — by
windowed cost, tie-broken by active sessions. This is how work sheds from Claude to
Gemini/Codex once Claude's usage runs ahead, keeping usage even. Toggle it globally
with the **Balance agent usage** setting.

**Models.** Model precedence when launching a session is: explicit per-task/per-session
model → per-agent configured model (Settings → Agents) → the agent's default. The
agent default is the latest Opus for Claude and each CLI's own latest model for
Gemini/Codex. You can set a model when creating a task, in a scheduled job's front
matter, or when sending a follow-up message into a session.
